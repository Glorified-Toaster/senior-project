...
func (h *UserHandler) GetUserByID() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err != nil {
			h.logger.LogErrorWithLevel("error", "DATABASE_ERROR", "DATABASE_ERROR", "Failed to get user by ID", err)
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get user by ID"})
			return
		}
...
func (h *UserHandler) GetUserByUsername() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err != nil {
			h.logger.LogErrorWithLevel("error", "DATABASE_ERROR", "DATABASE_ERROR", "Failed to get user by username", err)
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get user by username"})
			return
		}
