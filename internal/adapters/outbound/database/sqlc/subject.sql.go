...
DO UPDATE SET
  deleted_at = NULL,
  updated_at = NOW()
RETURNING subject_id, instructor_id, assigned_at, assigned_by, deleted_at, updated_at
`
...
DO UPDATE SET
  deleted_at = NULL,
  updated_at = NOW()
RETURNING subject_id, student_id, assigned_at, assigned_by, deleted_at, updated_at
`
...
func (q *Queries) GetSubjectByID(ctx context.Context, id uuid.UUID) (GetSubjectByIDRow, error) {
	...
	err := row.Scan(
		...
		&i.Subject.DurationMinutes,
		&i.Subject.TotalMarks,
		&i.Subject.PassScore,
		&i.Subject.Status,
		&i.Subject.CreatedAt,
		&i.Subject.UpdatedAt,
...
func (q *Queries) ListAllSubjects(ctx context.Context, arg ListAllSubjectsParams) ([]ListAllSubjectsRow, error) {
	rows, err := q.db.Query(ctx, listAllSubjects, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListAllSubjectsRow
	for rows.Next() {
...
type ListSubjectsForInstructorRow struct {
	Subject       Subject `json:"subject"`
	TotalEnrolled int64   `json:"total_enrolled"`
}

func (q *Queries) ListSubjectsForInstructor(ctx context.Context, instructorID uuid.UUID) ([]ListSubjectsForInstructorRow, error) {
	rows, err := q.db.Query(ctx, listSubjectsForInstructor, instructorID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSubjectsForInstructorRow
	for rows.Next() {
		var i ListSubjectsForInstructorRow
		if err := rows.Scan(
			&i.Subject.ID,
			&i.Subject.Title,
			&i.Subject.Description,
...
type ListSubjectsForStudentRow struct {
	Subject       Subject `json:"subject"`
	TotalEnrolled int64   `json:"total_enrolled"`
}

func (q *Queries) ListSubjectsForStudent(ctx context.Context, studentID uuid.UUID) ([]ListSubjectsForStudentRow, error) {
	rows, err := q.db.Query(ctx, listSubjectsForStudent, studentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListSubjectsForStudentRow
	for rows.Next() {
		var i ListSubjectsForStudentRow
		if err := rows.Scan(
			&i.Subject.ID,
			&i.Subject.Title,
			&i.Subject.Description,
...
UPDATE subject_instructors SET deleted_at = NOW() WHERE subject_id = $1 AND instructor_id = $2 RETURNING subject_id, instructor_id, assigned_at, assigned_by, deleted_at, updated_at
`
...
UPDATE subject_students SET deleted_at = NOW() WHERE subject_id = $1 AND student_id = $2 RETURNING subject_id, student_id, assigned_at, assigned_by, deleted_at, updated_at
`
